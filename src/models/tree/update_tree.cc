...
struct update_tree_fn {
  ...
  void operator()(legate::TaskContext context)
  {
    ...
    auto l2_regularization = context.scalars().at(0).value<double>();
    auto l1_regularization = context.scalars().at(1).value<double>();
    ...
    for (int i = 0; i < num_nodes; i++) {
      for (int j = 0; j < num_outputs; j++) {
        ...
        if (H > 0.0) {
          new_leaf_value[{i, j}] =
            CalculateLeafValue(new_gradient[{i, j}], H, l2_regularization, l1_regularization);
        } else {
          new_leaf_value[{i, j}] = 0.0;
        }
