...
void serialise_sketches(std::vector<datasketches::kll_sketch<float>> &sketches,  legate::Store &output){
  std::size_t size = 0;
 ...
 std::vector<datasketches::kll_sketch<float>> deserialise_sketches(const uint8_t *in, std::size_t size){
  std::size_t offset = 0;
...
class QuantileTask : public Task<QuantileTask, QUANTILE> {
 public:
  static void cpu_variant(legate::TaskContext& context)
  {
    ...
    std::vector<datasketches::kll_sketch<float>> sketches(n_features, datasketches::kll_sketch<float>(kSketchK));
    for (size_t j = shape.lo[1]; j <= shape.hi[1]; ++j) {
      for (size_t i = shape.lo[0]; i <= shape.hi[0]; ++i) { sketches[j - shape.lo[1]].update(in[i][j]); }
    }
...
std::vector<float> get_deduplicated_cuts(const datasketches::kll_sketch<float>&sketch, std::size_t n_bins){
...
class QuantileOutputTask : public Task<QuantileOutputTask, QUANTILE_OUTPUT> {
 public:
  static void cpu_variant(legate::TaskContext& context)
  {
    ...
    auto sketches = deserialise_sketches(serialised_sketch.read_accessor<uint8_t, 1>().ptr(0), serialised_sketch.shape<1>().volume());
    ...
    // Return quantiles
    auto cuts_result = cuts_output.create_output_buffer<float, 1>(legate::Point<1>(cuts.size()), true);
    std::copy(cuts.begin(), cuts.end(), cuts_result.ptr(0));
    auto ptr_result = ptr_output.create_output_buffer<uint64_t, 1>(legate::Point<1>(ptr.size()), true);
  ...
  static void cpu_variant(legate::TaskContext& context)
  {
    ...
    for (std::size_t i = 0; i < n_rows; i++) {
      for (std::size_t j = 0; j < n_features; j++) {
        out[legate::Point<2>(i, j)] = 
          (std::lower_bound(
             quantiles_iter.ptr(ptr_iter[j]), quantiles_iter.ptr(ptr_iter[j + 1]), in[legate::Point<2>(i, j)]) -
